# java-notes
[toc]
# 算法

> [8 种排序](https://www.cnblogs.com/IUbanana/p/7056734.html)

# 数据结构

> [算法和数据结构](./docs/算法和数据结构.md)
>
> [30张图带你彻底理解红黑树](https://www.jianshu.com/p/e136ec79235c)

# 设计模式

> [设计模式](./docs/设计模式/设计模式.md)
>
> [单例模式](./docs/设计模式/单例模式.md)
>
> [装饰者模式](./docs/设计模式/装饰者模式)
>
> [观察者模式](./docs/设计模式/观察者模式)

# 基础

> [8 种基础数据类型和 String、StringBuffer、StringBuilder](./docs/数据类型.md) 
>
> [修饰符](./docs/修饰符.md)
>
> [类与对象](./docs/类与对象.md)
>
> [Java8](./docs/JAVA8.md)
>
> [keytool](./docs/keytool.md)

- 集合

> [ArrayList 源码](./docs/ArrayList.md)
>
> [LinkedList 源码](./docs/LinkedList.md)
>
> [HashMap 源码](./docs/HashMap.md)
>
> [LinkedHashMap](./docs/LinkedHashMap.md)
>
> ConcurrentHashMap
>
> [TreeMap](./docs/TreeMap.md)

- IO

> [IO](./docs/IO.md)
>
> [Java NIO浅析](https://zhuanlan.zhihu.com/p/23488863)
>
> [Java NIO 系列教程](http://ifeve.com/java-nio-all/)
>
> [漫话：如何给女朋友解释什么是Linux的五种IO模型？](https://mp.weixin.qq.com/s?__biz=Mzg3MjA4MTExMw==&mid=2247484746&idx=1&sn=c0a7f9129d780786cabfcac0a8aa6bb7&source=41#wechat_redirect)
>
> [多路复用之select、poll、epoll](https://www.cnblogs.com/aspirant/p/9166944.html)
>
> [Netty 入门与实战：仿写微信 IM 即时通讯系统](https://juejin.im/book/5b4bc28bf265da0f60130116/section)
>
> [Netty中如何解决select空轮询](https://blog.csdn.net/qq_41884976/article/details/91913820)

- 并发

> [多线程](./docs/多线程.md)
>
> [Java 并发](https://github.com/CyC2018/CS-Notes/blob/master/notes/Java 并发.md)
>
> [synchronized 原理](./docs/synchronized.md)
>
> [volatile 原理](./docs/volatile.md)
>
> [Java 并发基础之内存模型](https://javadoop.com/post/java-memory-model)
>
> [ReentrantLock 实现原理](https://github.com/crossoverJie/JCSprout/blob/master/MD/ReentrantLock.md)
>
> AQS 源码
>
> > - [一行一行源码分析清楚 AbstractQueuedSynchronizer](https://javadoop.com/post/AbstractQueuedSynchronizer)
> > - [一行一行源码分析清楚 AbstractQueuedSynchronizer（二）](https://javadoop.com/post/AbstractQueuedSynchronizer-2)
> > - [一行一行源码分析清楚 AbstractQueuedSynchronizer（三）](https://javadoop.com/post/AbstractQueuedSynchronizer-3)
>
> [Java7/8 中的 HashMap 和 ConcurrentHashMap 全解析](https://javadoop.com/post/hashmap)
>
> [JAVA中写时复制(Copy-On-Write)Map实现](https://www.cnblogs.com/hapjin/p/4840107.html)
>
> [CountDownLatch](https://cloud.tencent.com/developer/article/1038486)
>
> [CyclicBarrier](https://cloud.tencent.com/developer/article/1186610)
>
> [Semaphore](https://cloud.tencent.com/developer/article/1129580) 
>
> [Exchanger](https://cloud.tencent.com/developer/article/1074996)
>
> [线程池是如何实现调用线程后线程不结束](https://blog.csdn.net/qq_43171869/article/details/86601425)
>
> [从线程池理论聊聊为什么要看源码](https://mp.weixin.qq.com/s/alQvxhInQvBv1MsXdXnOkA)
>
> [wait和notifyAll实现生产消费模型](https://www.cnblogs.com/Kevin-mao/p/5950743.html)
>
> [线程死锁](./docs/死锁.md)

- JVM

> [JVM](./docs/JVM/JVM.md)
>
> [class 文件](https://www.jianshu.com/p/247e2475fc3a)
>
> [虚拟机自带性能监控和故障排查工具](./docs/JVM/虚拟机自带性能监控和故障排查工具.md)
>
> [解密阿里线上问题诊断工具Arthas和jvm-sandbox](https://github.com/javagrowing/JGrowing/blob/master/CaseStudy/排查工具/解密阿里线上问题诊断工具Arthas和jvm-sandbox.md)
>
> [Arthas](./docs/JVM/arthas.md)
>
> [类加载器](./docs/JVM/类加载器.md)

# web 基础

>[Servlet](https://blog.csdn.net/qq_19782019/article/details/80292110)
>
>[HttpServletRequest](https://mp.weixin.qq.com/s/zdOsxUW3Iiy_sHxiT-E0HQ)
>
>[Listener](https://www.cnblogs.com/ygj0930/p/6374384.html)
>
>[Filter](https://mp.weixin.qq.com/s/5h8NuasWIMr7yVTCGFv9oA)
>
>[Tomcat如何处理一个请求](https://www.cnblogs.com/crazylqy/p/4706223.html)
>
>[Tomcat系列文章](https://mp.weixin.qq.com/s/kgcr7zdcRlJNHG84ODgHDw)
>
>[NGINX 简介](./docs/nginx/NGINX 简介.md)
>
>[安装NGNIX](./docs/nginx/安装NGNIX.md)
>
>[NGINX常用命令](./docs/nginx/常用命令.md)

# 框架

## Spring
>
>  [观察 Spring bean 实例化](./docs/spring/观察 Spring bean 实例化.md)
> 
>  [Spring IOC 容器源码分析](https://javadoop.com/post/spring-ioc)
> 
>  [Spring AOP 使用介绍，从前世到今生](https://javadoop.com/post/spring-aop-intro)
> 
>  [Spring AOP 源码解析](https://javadoop.com/post/spring-aop-source)
> 
>  [AOP 在哪个生命周期完成的](https://www.jianshu.com/p/83f45b8ed3a6)
> 
>  [理解BeanDefinition和BeanPostProcessor](https://zhuanlan.zhihu.com/p/70642885)
> 
>  [spring 中的设计模式](./docs/Spring 中的设计模式.md)
> 
>  [动态代理和CGLIB](./docs/动态代理和CGLIB.md)
> 
> > [事务传播机制](./docs/spring事务传播机制.md)
>
> Spring MVC
>
> > [SpringMVC源码分析](https://blog.csdn.net/gududedabai/article/details/83352106)
>
> Mybatis
>
> > [Mapper 加载流程](./docs/Mybatis加载流程.md)
> >
> > [Plugin原理](./docs/Mybatis插件原理.md)

# 数据库

## Redis
>  [布隆过滤器](./docs/布隆过滤器.md)
> 
>  [Redis数据类型](./docs/Redis数据类型)
> 
>  [面试官：你看过Redis数据结构底层实现吗？](https://www.cnblogs.com/javazhiyin/p/11063944.html)
> 
>  [Redis 数据结构和对象系统，有这 12 张图就够了！](https://blog.csdn.net/TG229dvt5I93mxaQ5A6U/article/details/93259425)
> 
>  为什么单线程
> 
>  内存操作、线程切换消耗、多路复用
> 
>  [Redis 深度历险：核心原理与应用实践](https://juejin.im/book/5afc2e5f6fb9a07a9b362527)
> 
>  应用4 HyperLogLog
> 
>  应用5 布隆过滤器
> 
>  应用6 简单限流
> 
>  应用7 漏斗限流
> 
>  原理3 持久化（AOF 和 RDB）
> 
>  [过期策略](./docs/Redis过期策略.md)
> 
>  [Redlock分布式锁](https://github.com/Snailclimb/JavaGuide/blob/master/docs/database/Redis/Redlock分布式锁.md)
> 
>  集群、哨兵
> 
>  集群模式下的发布订阅模式失效
> 
>  6节点，三主（哨兵）三备
> 
## MySQL
>
> >[一千行MySQL命令](https://github.com/Snailclimb/JavaGuide/blob/master/docs/database/一千行MySQL命令.md)
> >
> >[存储引擎InnoDB 和 MyISAM](./docs/存储引擎InnoDB和MyISAM.md)
> >
> >[InnoDB 中 B + Tree 索引](https://github.com/crossoverJie/JCSprout/blob/master/MD/MySQL-Index.md)
> >
> >[SQL 执行顺序](./docs/SQL执行顺序.md)
> >
> >[一条sql语句在mysql中如何执行的](https://github.com/Snailclimb/JavaGuide/blob/master/docs/database/一条sql语句在mysql中如何执行的.md)
> >
> >[理解事物的4种隔离机制](https://blog.csdn.net/qq_33290787/article/details/51924963)
> >
> >[轻松理解MYSQL MVCC 实现机制](https://blog.csdn.net/whoamiyang/article/details/51901888)
> >
> >[redo log 和 binlog 实现事务](https://blog.csdn.net/hanpeiyu1995/article/details/90044300)
> >
> >[索引和锁](https://juejin.im/post/5b55b842f265da0f9e589e79#heading-2)
> >
> >[Innodb锁机制：Next-Key Lock 浅谈](https://www.cnblogs.com/zhoujinyi/p/3435982.html)
> >
> >[Delete&Insert引发的Mysql死锁](https://blog.csdn.net/hellozhxy/article/details/80455092)
> >
> >[SQL 优化](./docs/SQL优化.md)
> >
> >[explain](https://blog.csdn.net/drdongshiye/article/details/84546264)
> >
> >[主从复制](https://github.com/ZXZxin/ZXBlog/blob/master/数据库/MySQL/advance/MYSQL主从复制.md)
>
## MongoDB
>
> > [Mongodb相对于关系型数据库的优缺点](http://mxdxm.iteye.com/blog/2093603)
> >
> > [MongoDb 基础语法](https://www.mongodb.org.cn/tutorial/)

# 分布式

> [分布式](https://github.com/CyC2018/CS-Notes/blob/master/notes/分布式.md)
>
> [集群](https://github.com/CyC2018/CS-Notes/blob/master/notes/集群.md)
>
> [缓存](https://github.com/CyC2018/CS-Notes/blob/master/notes/缓存.md)
>
> [无锁队列Disruptor](https://github.com/javagrowing/JGrowing/blob/master/常用框架/其他框架/无锁队列Disruptor.md)
>
> [深入解密来自未来的缓存-caffeine](https://github.com/javagrowing/JGrowing/blob/master/常用框架/其他框架/深入解密来自未来的缓存-Caffeine.md)
>
> [浅析如何设计一个亿级网关](https://github.com/javagrowing/JGrowing/blob/master/服务端开发/浅析如何设计一个亿级网关.md)
>
> [如果再有人问你分布式ID，这篇文章扔给他](`https://github.com/javagrowing/JGrowing/blob/master/分布式/如果再有人问你分布式 ID，这篇文章丢给他.md`)
>
> [如果再有人问你分布式锁,这篇文章扔给他](`https://github.com/javagrowing/JGrowing/blob/master/分布式/再有人问你分布式锁，这篇文章扔给他.md`)
>
> [谈谈数据一致性](https://github.com/javagrowing/JGrowing/blob/master/分布式/谈谈数据一致性.md)
>
> [分布式必看系列](https://github.com/doocs/advanced-java)
>
> [6种限流方式](./docs/分布式/6种限流方式.md)

# 微服务

## SpringBoot
>
> > [SpringApplication.run做了哪些事？](https://mp.weixin.qq.com/s/O2zS2yyLy1Vu_aPtR28wiQ)
> >
> > [SpringBoot中自动配置原理](https://mp.weixin.qq.com/s/Og1fuus07h2Gc6TvziSJYA)
> >
> > [SpringBoot自定义 Starter](./docs/SpringBoot自定义 Starter.md)	
> >
> > [SpringBoot使用Docker快速部署项目](https://juejin.im/post/5d26b1efe51d4556da53d159)
>
## SpringCloud

### 注册中心

### 负载均衡

### RPC

### 熔断器

### 网关

### 监控中心

### 链路追踪

## Dubbo
>
> [Eureka-注册中心](./docs/Eureka.md)
>
> [Zookeeper](./docs/Zookeeper.md)-注册中心、配置中心
>
> Nacos-注册中心、配置中心
>
> Ribbon-负载均衡
>
> Feign-声明式服务调用
>
> Apoll-配置中心
>
> Spring Cloud Config-配置中心
>
> Hystric-熔断器
>
> Sentinel-熔断器
>
> Spring Cloud Gateway-网关
>
> Zuul-网关
>
> Spring cloud Stream-消息驱动
>
> Spring Cloud Bus-消息总线
>
> Spring Cloud Security-认证与授权
>
> Spring Cloud Sleuth-链路追踪
>
> ELK-elasticsearch+logstash+kibana
>
> [一致性哈希算法](./docs/一致性哈希算法.md)

# 中间件

## RabbitMQ
>
> > [RabbitMQ的应用场景以及基本原理介绍](https://blog.csdn.net/whoamiyang/article/details/54954780)
> >
> > [RabbitMQ、 ActiveMQ、RocketMQ、Kafka 的比较](https://github.com/doocs/advanced-java/blob/master/docs/high-concurrency/why-mq.md)
> >
> > [RabbitMQ的五种模式](https://www.cnblogs.com/Alva-mu/p/9535396.html)
> >
> > [RabbitMQ的应用场景以及基本原理介绍](https://blog.csdn.net/whoamiyang/article/details/54954780)
> >
> > [基于RabbitMQ的死信实现延迟队列](https://www.liangzl.com/get-article-detail-37702.html)
> >
> > [docker简易搭建RabbitMQ集群](https://blog.csdn.net/belonghuang157405/article/details/83540148)
> >
> > [如何保证消息不被重复消费？（如何保证消息消费的幂等性）](https://github.com/doocs/advanced-java/blob/master/docs/high-concurrency/how-to-ensure-that-messages-are-not-repeatedly-consumed.md)
> >
> > [如何保证消息的可靠性传输？（如何处理消息丢失的问题）](https://github.com/doocs/advanced-java/blob/master/docs/high-concurrency/how-to-ensure-the-reliable-transmission-of-messages.md)
>
## Kafka
>
>  概念：topic、region、节点
> 
>  消息模式

## Active
## RocketMQ

# 大数据

> Hadoop
>
> Hive
>
> Hbase
>
> Storm
>
> Spark

# 服务器

> Linux
>
> shell
>
> docker

# 版本控制工具

> SVN
>
> [GIT常用命令](./docs/git/git.md)

# 编译工具

## Maven

## 自动持续集成

### Jenkins 一键部署
### GitLab

## 其他

- 提高开发效率

> [Lombok详解](./docs/Lombok详解.md) 为 `bean` 自动生成 `get`、`set`、构造方法等。

- 写笔记博客

> 印象笔记 谷歌插件非常好用
> [Gitee Pages + GitBook + Markdown](./docs/GiteePages+GitBooks+MarkDown.md)

- 其他语言

> [Node.js](./docs/node/node目录.md)
>
> [vue](./docs/前端/vue.md)
>
> 



