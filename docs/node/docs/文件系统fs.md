...
`WriteStream` 是一个[可写流](http://nodejs.cn/s/9JUnJ8)。
...
`mode` 设置文件模式（权限和粘滞位），但仅限于创建文件的情况。 在 Windows 上，只能操作写权限，参阅 [`fs.chmod()`](http://nodejs.cn/s/6QUJs8)。
...
## fs.rmdir(path, callback)[#](http://nodejs.cn/api/fs.html#fs_fs_rmdir_path_callback)

[中英对照](http://nodejs.cn/api/fs/fs_rmdir_path_callback.html)[提交修改](https://github.com/nodejscn/node-api-cn/edit/master/fs/fs_rmdir_path_callback.md)
...
## fs.rmdirSync(path)[#](http://nodejs.cn/api/fs.html#fs_fs_rmdirsync_path)

[中英对照](http://nodejs.cn/api/fs/fs_rmdirsync_path.html)[提交修改](https://github.com/nodejscn/node-api-cn/edit/master/fs/fs_rmdirsync_path.md)
...
异步的 [`symlink(2)`](http://nodejs.cn/s/th7NwY)。 除了可能的异常，完成回调没有其他参数。 `type` 参数仅在 Windows 上可用，在其他平台上会被忽略。 它可以被设置为 `'dir'`、 `'file'` 或 `'junction'`。 如果未设置 `type` 参数，则 Node 将会自动检测 `target` 的类型并使用 `'file'` 或 `'dir'`。 如果 `target` 不存在，则将会使用 `'file'`。 Windows 上使用 `'junction'` 要求目标路径是绝对路径。 当使用 `'junction'` 时， `target` 参数将自动标准化为绝对路径。
...
`fs.unlink()` 不能用于目录。 要删除目录，则使用 [`fs.rmdir()`](http://nodejs.cn/s/4snHRH)。
...
- 在 SunOS 系统上（包括 Solaris 和 SmartOS），使用[事件端口](http://nodejs.cn/s/yqJ48A)。
...
### fsPromises.rmdir(path)[#](http://nodejs.cn/api/fs.html#fs_fspromises_rmdir_path)

[中英对照](http://nodejs.cn/api/fs/fspromises_rmdir_path.html)[提交修改](https://github.com/nodejscn/node-api-cn/edit/master/fs/fspromises_rmdir_path.md)
